...
2. `media-tools/.claude-plugin/plugin.json` → `"version"` field
3. `.cursor-plugin/plugin.json` → `"version"` field
4. `.claude-plugin/marketplace.json` → `metadata.version` field
5. `media-tools/skills/pretext/SKILL.md` → `metadata.version` in YAML frontmatter
6. `media-tools/skills/youtube-downloader/SKILL.md` → `metadata.version` in YAML frontmatter
7. `media-tools/skills/pretext/VERSION` — copied from root
8. `media-tools/skills/youtube-downloader/VERSION` — copied from root
